...
async function createNote (userId, noteAlias, permission = config.defaultPermission) {
...
async function showNote (req, res) {
  ...
  const notePermission = req.query && req.query.mode ? config.permission[req.query.mode] : null;
  ...
  if (!note) {
    ...
    note = await createNote(userId, noteId, notePermission)
  }
