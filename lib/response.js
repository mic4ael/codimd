...
function newNote (req, res, next) {
  ...
  if (req.query && req.query.mode && req.query.mode in config.permission) {
    notePermission = config.permission[req.query.mode];
  }
  models.Note.create({
    ownerId: owner,
    alias: req.alias ? req.alias : null,
    content: body,
    permission: notePermission ? notePermission : null,
  }).then(function (note) {
    if (req.isAuthenticated()) {
